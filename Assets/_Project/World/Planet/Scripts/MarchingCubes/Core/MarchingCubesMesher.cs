...
namespace _Project.World.Planet.Scripts.MarchingCubes.Core
{
    public static class MarchingCubesMesher
    {
        ...
        public static void GenerateAt(int3 pos, DensityField grid, MeshDataBuilder meshDataBuilder)
        {
            ...
            Vtx[] vertices = new Vtx[12]; // the resulting vertices can be 12 max since there are only 12 edges. we fill the rest of the values with -1.
            ...
            {
                vertices[0].Pos =
                    VertexInterp(IsoLevel, p0, p1, // so we know now that this edge is used but if we just pass the center between the start and end point of that edge it would be way to blocky. so we just interpolate between the 2 densities of the corresponding values. this way everything is distributed smoothly.
        ...
        private static int GetCubeIndexAt(int3 pos, DensityField grid, float isoLevel)
        {
            int cubeIndex = 0;
