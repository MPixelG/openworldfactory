...
namespace _Project.World.Planet.Scripts.MarchingCubes.BurstMeshGeneration
{
    ...
    public unsafe struct BurstMeshGeneratorJob : IJob
    {
        ...
        private void GenerateAt(int3 pos, DensityFieldData grid)
        {
            ...
            int
                edgeFlags = EdgeTable[cubeIndex]; // converts the given corner configuration to a binary number where every bit represents one edge. note that we convert an 8 bit number (one bit for every corner) to a 12 bit number (one bit for every edge). 
