...
namespace _Project.World.Planet.Scripts.Chunking.OctreeChunkSystem.v2
{
    ...
    {
        public ulong MortonCode; // the morton code of the node is a unique identifier for the node that encodes its position in the octree. it is calculated by using 3 bits per layer.
                                 ...
                                 // we can resolve that problem by adding a trailing 1 at the end of our code. so it would be 0b0000001 and 0000000001, so the computer knows the difference. 


        public int
            FirstChildIndex; // since the nodes are stored linear in one big list on one layer we can store the index of the first child
