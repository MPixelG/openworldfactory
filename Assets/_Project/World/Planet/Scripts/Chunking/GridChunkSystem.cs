...
namespace _Project.World.Planet.Scripts.Chunking
{
    ...
    public class GridChunkSystem : ChunkSystem
    {
        ...
        [SerializeField] private MarchingCubesChunk chunkPrefab; // this is basically the pattern for a chunk. the chunk system will instantiate this prefab for every chunk it needs to create and then configure it with the right settings
        ...
        [SerializeField, Range(0f, 1f)] private float isoLevel = 0.5f; // the isoLevel used to generate the chunks
        ...
        private MarchingCubesChunk _lastChunkPrefab;
        ...
        {
            _chunks.Clear();
            foreach (var chunk in GetComponentsInChildren<MarchingCubesChunk>(true))
            {
        ...
        private void EnsureChunkAt(Vector3Int coord)
        {
            ...
            {
                if (existing is not MarchingCubesChunk existingChunk) return;
            ...
            MarchingCubesChunk chunk = Instantiate(chunkPrefab, transform);
